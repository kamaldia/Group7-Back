...
const updateUser = async (req, res) => {
  ...
  try {
    ...
    const updatedWithPass = await User.update({username: updatedUser.username, password: hashedPassword}, { where: { id } } )
    res.status(200).json({ message: "User updated", updatedWithPass });
  } catch (error) {
    console.error(error);
